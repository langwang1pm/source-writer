...
import { Plus, MessageSquare, Settings, Building2, Tag } from "lucide-react";
      ...
      {showCreate && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 32, width: 400, maxWidth: "90vw" }}
            ...
            <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 20 }}>创建项目空间</h2>
            <select
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button onClick={() => setShowCreate(false)} style={{ padding: "8px 20px", borderRadius: 8, border: "1px solid #ddd", background: "#fff", cursor: "pointer", fontSize: 14 }}>取消</button>
              <button onClick={handleCreate} style={{ padding: "8px 20px", borderRadius: 8, border: "none", background: "#1a1a2e", color: "#fff", cursor: "pointer", fontSize: 14 }}>创建</button>
            </div>
          </div>
        </div>
      )}
